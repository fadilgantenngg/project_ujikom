...
<div class="container">
    ...
    <form action="{{ route('film.update', $film->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            @if ($film->poster)
                <div class="mt-2">
                    <img src="{{ asset('images/films/' . $film->poster) }}" alt="Poster" style="width: 100px; height: auto;">
                </div>
            @endif
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.1" class="form-control @error('rating') is-invalid @enderror" id="rating" name="rating" value="{{ old('rating', $film->rating) }}">
            @error('rating')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
